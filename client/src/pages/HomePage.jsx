...
const HomaPage = () => {
  ...
  useEffect(() => {
    // Simulando la carga de recursos
    const timer = setTimeout(() => {
      setLoading(false); // Cambia el estado de carga después de que se carguen los recursos
  ...
  return (
    <div>
      {loading ? (
        <Loader /> // Muestra el componente de carga mientras se carga
      ) : (
      <div className="scroll-snap-container flex flex-col pt-10">
        ...
        <video
          ...
          src={`${import.meta.env.BASE_URL}cruce.mp4`}
          muted
          autoPlay
          loop
          preload="metadata"
